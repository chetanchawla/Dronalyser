...
def ndvi(img,
    ...
    #fig.clf()
    #Capturing subsequent images
    camera.start_preview()
    sleep(2)
    camera.capture(stream, format='jpeg')
    # "Rewind" the stream to the beginning so we can read its content
    stream.seek(0)
    baseimage = Image.open(stream)
    plt.close('all')
    gc.collect()
...
baseimage = Image.open(stream)
while(True):
    #name='filterpark'+str(i*2)+'.jpg'
    ndvi(baseimage,'test_ndvi8.png',show_histogram = True,)
    i=i+1
